...
void handle_call(TrunkMessage message, System *sys) {
  ...
  for (vector<Call *>::iterator it = calls.begin(); it != calls.end();) {
    ...
    if ((call->get_talkgroup() == message.talkgroup) && (call->get_sys_num() == message.sys_num)) {
      call_found = true;
      if (call->get_state() == recording) {
      BOOST_LOG_TRIVIAL(trace) << "[" << sys->get_short_name() << "]\tTG: " << call->get_talkgroup_display() << "\tFreq: " << FormatFreq(call->get_freq()) << "\tUpdate! " << FormatFreq(message.freq) << "\tElapsed: " << call->elapsed() << "s \tSince update: " << call->since_last_update() << "s";
      }
